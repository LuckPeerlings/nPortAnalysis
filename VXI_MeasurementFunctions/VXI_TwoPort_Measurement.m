...
elseif strcmp(reply,'N')
    ...
    % overridden
    MIC_RANGE = [10,10,10,10,10,10,10,10];
    REF_RANGE = [1,1];
    TEMP_RANGE = [1,1];
    ACCEL_RANGE =[];
    CTA_RANGE =[];
    ...
    SENSITIVITY_MIC_CHAN = [1 1 1 1 1 1 1 1]*3.16e-3;
    SENSITIVITY_REF_CHAN = [1 1];
    SENSITIVITY_TEMP_CHAN = [10.164 9.932]*1e-3;
    SENSITIVITY_ACCEL_CHAN = [];
    SENSITIVITY_CTA_CHAN = [];
    SENSITIVITY_OTHER_CHAN = [9.1526]*1e-4;
    
    GAIN_ACCEL_CHAN = [];
    
    ID_MIC_CHAN = {'0020','0018','0017','0021','0001','0006','0004','0008'};
    ID_REF_CHAN = {'x','x'};
    ID_TEMP_CHAN = {'T001','T002'};
    ID_ACCEL_CHAN = {};
    ID_CTA_CHAN = {};
    ID_OTHER_CHAN = {'P001'};
    
    %What are the microphone positions?
    MIC_POSITION.Port1 = [310, 344, 378, 585]*1e-3;
    MIC_POSITION.Port2 = [310, 372.5, 460, 555]*1e-3;
    
    
    
    %A1.4 Wave Calibration
    %Use amplitude information from another file
    EXTERNAL_AMP_SCALE = 'False';
    DATAFILE = '..\MeasurementInfo.mat';
    
    %Parameters for the wave calibration, only one block of data is used to
    %determine the amplitude of the wave.
    CALWAVE = 'False';

    %Channels to use for the calibration
    MIC_CHAN_CALWAVE.Port1 = [1,2,3,4];
    MIC_CHAN_CALWAVE.Port2 = [5,6,7,8];
    
    %Positions of the microphones corresponding to the channels used in the
    %wave calibration.
    MIC_POS_CALWAVE.Port1 = [0, 55, 250]*1e-3;
    MIC_POS_CALWAVE.Port2 = [0, 55, 250]*1e-3;
    %Velocity correction to determine the mean velocity in the measurement
    %port.
    VELOCITY_CORR_CAL.Port1 = -0.8575*1.05;
    VELOCITY_CORR_CAL.Port2 = 0.8575*1.05*(25e-3/45e-3)^2;
    ...
    AUTORANGE_BACKGROUND = 'True';
    MEASUREMENTTIME_BACKGROUND = 60;
...
email.APIkey  = 'key-e1333b9c1bd5a408347ec06e6485933e';
email.domain  = 'sandbox5f4f9d086cd04ba09fe70a763c96a054.mailgun.org';
email.from    = 'The seven MWL dwarves <postmaster@sandbox5f4f9d086cd04ba09fe70a763c96a054.mailgun.org>';
email.to      = 'pierre.chabrol@centrale-marseille.fr';
email.subject = 'Measurements finished Anechoic room';
email.text    = 'Come down to the place without echos, we are done!';
